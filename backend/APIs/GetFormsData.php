...
class GetFormsData {
    ...
    public function register_routes() {
        register_rest_route('awcontactform/v1', '/selectformsdata/', [
            'methods'               => \WP_REST_Server::READABLE,
            'callback'              => [$this, 'select_forms_data'],
    ...
    public function select_forms_data($request) {
        $page = isset($request['page']) && is_numeric($request['page']) ? intval($request['page']) : 1;
        $pageSize = isset($request['pageSize']) && is_numeric($request['pageSize']) ? intval($request['pageSize']) : -1;
